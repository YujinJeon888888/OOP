...
public class MyWin_battle_광산 extends JFrame {
	public 요술이 p1;
	public 해골 p2;
	 ...
	 */
	public MyWin_battle_광산(요술이 p1,해골 p2) {
		this();
	...
	public void action공격하기(Player p, Player target, JProgressBar target_bar) throws InterruptedException {
		...
		if(target.getHp()<=0) { 
			...
			if(p.이름=="해골"&&p1.get방어구()!=null) {//해골이 이기고 요술이가 갑옷이 있으면
				p1.set방어력(p1.get방어력()*0.95);//방어구 내구력을 5% 깎음
				JOptionPane.showMessageDialog(null, p.이름+"이(가) 이겨 "+target.이름+"의 방어력이 5%깎였습니다!");
			}
			if(p.이름=="요술이") {//요술이가 이기면
				p1.set돈주머니(p1.get돈주머니()+10000);//10000원 줌
				JOptionPane.showMessageDialog(null, p2.이름+"(와)과의 배틀에서 이겨 10,000원을 얻었습니다!");
			}	
		dispose();
		}				
	}
	...
	public MyWin_battle_광산() {
		...
		label2.setIcon(new ImageIcon(MyWin_battle_광산.class.getResource("/\uC774\uBBF8\uC9C0/skeleton.png")));
		...
		bar2.setValue(90);
		bar2.setForeground(new Color(204, 153, 255));
		...
		lblNewLabel.setIcon(new ImageIcon(MyWin_battle_광산.class.getResource("/\uC774\uBBF8\uC9C0/\uAD11\uC0B0.png")));
